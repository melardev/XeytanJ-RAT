...
class SpiResolver implements IAppDependencyResolver {
    ...
    public <S extends IService> S lookup(Class<S> clazz) {
        IService service = (services.stream().filter(s -> clazz.isAssignableFrom(s.getClassType())).findFirst().orElseThrow(NoSuchDependencyException::new));
        try { // This should always succeed
            return (S) service;
    ...
    public List<? extends IService> loadAllDependencies() {
