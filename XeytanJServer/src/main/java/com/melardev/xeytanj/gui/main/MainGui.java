...
public class MainGui extends JFrame implements IGui<MainUiListener> {
    ...
    JMenuItem menuUninstallServer;

    JMenuItem menuCloseConnection;
    ...
    public void display() {
        ...
        */
        ...
        /*
        Object[][] testData = {
                {"android_48", "albania", "66.243.2.1", "192.168.4.3", "Pcname", UUID.randomUUID()},
                {"windows_32", "Spain", "192.168.1.1", "192.168.1.1", "MyPc", UUID.randomUUID()}};

        modelTableClients = new DefaultTableModel(testData, columnNames);
        modelTableClients.addRow(new Object[]{"linux_32", "France", "192.168.1.2", "192.168.1.100", "LinuxPC", UUID.randomUUID()});
        */
...
*/
        ...
        menuCloseConnection.addActionListener(menuClientHandler);
        ...
        menuUninstallServer.addActionListener(menuClientHandler);
        ...
        mnBarListenTo = new JMenuItem("Listen to (Not Implemented)");
...
*/
    ...
    void onTurnOnOffDisplayClicked(UUID uuid, boolean on) {
        listener.onTurnOnOffDisplayClicked(uuid, on);
    }

    void onRebootClicked(UUID id) {
        listener.onRebootClicked(id);
    }

    void onShutdownClicked(UUID id) {
        listener.onShutdownClicked(id);
    }

    void onLogOffSystem(UUID id) {
        listener.onLogOffSystem(id);
    }

    void onLockSystemClicked(UUID id) {
        listener.onLockSystemClicked(id);
    }

    void onSessionKeyloggerClicked(UUID id) {
        listener.onSessionKeyloggerClicked(id);
    }

    void onVoIpRawClicked(UUID id) {
        listener.onVoIpRawClicked(id);
    }

    void onVoIpClicked(UUID id) {
        listener.onVoIpClicked(id);
    }

    void onStartChatClicked(UUID id) {
        listener.onStartChatRequested(id);
    }

    void onRemoteFileManagerClicked(UUID id) {
        listener.onRemoteFileManagerClicked(id);
    }

    void onRemoteDesktopClicked(UUID id) {
        listener.onRemoteDesktopClicked(id);
    }

    void onMenuCameraClicked(NetworkProtocol protocol, UUID id) {
        listener.onCameraStartRequested(protocol, id);
    }

    void onProcessListClicked(ProcessInfoDetails infoLevel, UUID id) {
        listener.onProcessListClicked(infoLevel, id);
    }

    void onRemoteShellClicked(UUID id) {
        listener.onRemoteShellRequested(id);
    }

    void onShowClientInfo(UUID id) {
        getListener().onShowClientInfo(id);
    }
    ...
    public void onCloseConnectionClicked(UUID uuid) {
        mediator.onCloseConnectionClicked(uuid);
    }

    public void onUninstallServer(UUID uuid) {
        mediator.onUninstallClicked(uuid);
    }
